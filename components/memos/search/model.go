...
func New(c *config.TomlConfig) (*Model, error) {
	...
	case tea.KeyMsg:
		...
		case "enter", "l":
			if m.focus == focusList && m.selected < len(m.results) {
				...
				components.OpenEditor(m.config.BaseDir, filePath)
				return m, nil
			}
		}
...
func (m Model) View() string {
	...
	if m.focus == focusInput {
		...
		help = "\n  ↑/k,↓/j: navigate | ctrl+u/ctrl+d: move by 5 | gg/G: top/bottom | enter/l: open | tab/ctrl+k: switch to search | esc: quit\n"
	}
...
func (m *Model) search() tea.Msg {
	...
	if query == "" {
		return searchResultMsg{results: []SearchResult{}}
	}

	memos, err := repos.NewMemoRepo(m.config.MemosDir()).MemoEntries()
	if err != nil {
		return searchResultMsg{results: []SearchResult{}}
	}
	...
	for _, memo := range memos {
		...
		matchesByType := make(map[MatchType][]Match)
		...
		for _, wordVariations := range wordQueries {
			...
			for _, q := range wordVariations {
				// Search by title
				if matches := SearchMemo(memo, q, MatchTitle); len(matches.Matches) > 0 {
					matchesByType[MatchTitle] = append(matchesByType[MatchTitle], matches.Matches...)
				}

				// Search by category
				if matches := SearchMemo(memo, q, MatchCategory); len(matches.Matches) > 0 {
					matchesByType[MatchCategory] = append(matchesByType[MatchCategory], matches.Matches...)
				}

				// Search by heading
				if matches := SearchMemo(memo, q, MatchHeading); len(matches.Matches) > 0 {
					matchesByType[MatchHeading] = append(matchesByType[MatchHeading], matches.Matches...)
				}

				// Search by content
				if matches := SearchMemo(memo, q, MatchContent); len(matches.Matches) > 0 {
					matchesByType[MatchContent] = append(matchesByType[MatchContent], matches.Matches...)
				}
			}
		}
		...
		for _, matches := range matchesByType {
			...
			uniqueMatches := make(map[string]Match)
	...
	for _, result := range resultsByFile {
		...
		sort.Slice(result.Matches, func(i, j int) bool {
			...
			if result.Matches[i].Type == MatchContent {
...
func (m Model) renderResults() string {
	...
	for i, result := range m.results {
		...
		matchesByType := make(map[MatchType]map[string]Match)
		for _, match := range result.Matches {
			if matchesByType[match.Type] == nil {
				matchesByType[match.Type] = make(map[string]Match)
			}
			...
			if len(matches) > 0 {
				...
				if mt.Type == MatchContent {
					...
					for _, match := range sortedMatches {
						switch match.Type {
						case MatchTitle, MatchCategory, MatchHeading:
...
func RenderResults(results []SearchResult, selected int, query string, romajiConv *RomajiConverter) string {
	var s strings.Builder
	s.Grow(4096) // Preallocate initial capacity
