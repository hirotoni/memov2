...
func GenerateTodoFile(c config.TomlConfig, truncate bool) error {
	...
	now := time.Now()
	repo := repos.NewFileRepo(c.TodosDir())
...
func inheritTodos(dir string, today time.Time, daysToSeek int) (models.FileInterface, error) {
	repo := repos.NewFileRepo(dir)
...
func findPrevTodosFile(baseDir string, today time.Time, daysToSeek int) (models.FileInterface, error) {
	repo := repos.NewFileRepo(baseDir)

	var found models.FileInterface
