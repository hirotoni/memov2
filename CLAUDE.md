...
cmd/             → Cobra CLI commands
  cmd/app/       → InitializeApp() — composition root called from each subcommand
  cmd/memos/     → browse, categories, index, list, new, open, rename, search, weekly
  cmd/todos/     → new, weekly
  cmd/config/    → show, edit
...
internal/utils/      → Markdown parsing/building helpers (yuin/goldmark + custom mygoldmark)
...
- **Testing**: Tests use testify assertions. Repos and services have `test_helpers_test.go` files (test-only helpers must use the `_test.go` suffix so they stay out of the production build) and `testdata/` directories with `*.golden` files for expected output.
...
This project follows Kiro-style spec-driven development (see `AGENTS.md` for the workflow: requirements → design → tasks → impl, with human review at each phase). When `.kiro/specs/{feature}/` or `.kiro/steering/` exist, read them for context; if they don't, the workflow has not been used yet for the current task.
