...
func (h *MarkdownHandler) HeadingBlockByHeading(source []byte, heading Heading) (*domain.HeadingBlock, error) {
	...
	markdownEntity := &domain.HeadingBlock{
...
func (h *MarkdownHandler) HeadingBlocksByLevel(source []byte, level int) ([]*domain.HeadingBlock, error) {
	reader := text.NewReader(source)
	doc := h.md.Parser().Parse(reader)
	...
	res := make([]*domain.HeadingBlock, len(foundNodes))
...
func (h *MarkdownHandler) TopLevelBodyContent(source []byte) *domain.HeadingBlock {
	reader := text.NewReader(source)
	doc := h.md.Parser().Parse(reader)
