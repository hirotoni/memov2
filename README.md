...
# カテゴリを指定してメモを作成
memov2 memos new "設計メモ" --category work/projects

# カテゴリ一覧を表示（fzf/peco との連携用）
memov2 memos categories

# カテゴリを選択してメモを作成（fzf/peco 連携）
# fzf/peco をキャンセル（空文字）した場合はカテゴリなしのメモが作成されます
memov2 memos new "議事録" --category "$(memov2 memos categories | fzf)"
memov2 memos new "議事録" --category "$(memov2 memos categories | peco)"
...
# カテゴリを選択してメモを作成（fzf をキャンセルするとカテゴリなしで作成）
memo-new() {
  local category
