...
func TestGenerateMemoFile_Success(t *testing.T) {
	...
	// Execute
	err = uc.GenerateMemoFile("Test Memo Title", []string{})
...
func TestGenerateMemoFile_WithCategory(t *testing.T) {
	// Setup
	tmpDir := t.TempDir()
	opts := toml.Option{BaseDir: tmpDir}
	cfg, err := toml.NewConfig(opts)
	...
	// Execute
	err = uc.GenerateMemoFile("Categorized Memo", []string{"work", "projects"})
	...
	call := mockEditor.Calls[0]
	assert.Contains(t, call.Path, "work/projects")
	assert.Contains(t, call.Path, "memo")
	assert.Contains(t, call.Path, ".md")
}
...
func TestGenerateMemoFile_EditorError(t *testing.T) {
	...
	// Execute
	err = uc.GenerateMemoFile("Test Memo", []string{})
