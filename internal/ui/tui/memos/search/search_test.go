...
// TestModel_StandaloneFzfNavigation verifies the fzf-style interaction used by
// `alt search`: focus stays in the input, ctrl+n/ctrl+p move the highlight, and
// enter selects-and-quits (reporting the path) instead of switching focus.
func TestModel_StandaloneFzfNavigation(t *testing.T) {
	tempDir := t.TempDir()
	cfg, err := toml.NewConfig(toml.Option{
		BaseDir:         tempDir,
		MemosFolderName: "memos/",
	...
	editor := &mock.MockEditor{}
	modelPtr, err := NewStandalone(cfg, editor)
	require.NoError(t, err)
	model := *modelPtr
	...
	// ctrl+n moves down while focus remains on the input.
	updatedModel, _ := model.Update(tea.KeyMsg{Type: tea.KeyCtrlN})
	model = updatedModel.(Model)
	assert.Equal(t, 1, model.selected, "ctrl+n should move the highlight down")
	assert.Equal(t, focusInput, model.focus, "focus should stay on the input")

	// A non-key message (e.g. cursor blink) must NOT re-run search or reset the
	// highlight back to the first element.
	type blinkMsg struct{}
	updatedModel, _ = model.Update(blinkMsg{})
	model = updatedModel.(Model)
	assert.Equal(t, 1, model.selected, "non-input messages must not reset the highlight")

	// ctrl+p moves back up.
	updatedModel, _ = model.Update(tea.KeyMsg{Type: tea.KeyCtrlP})
	model = updatedModel.(Model)
	assert.Equal(t, 0, model.selected, "ctrl+p should move the highlight up")

	// enter selects the highlighted file and quits (does not switch focus).
	updatedModel, cmd := model.Update(tea.KeyMsg{Type: tea.KeyEnter})
	model = updatedModel.(Model)
	assert.NotNil(t, cmd, "enter should return a command (quit)")
	assert.Equal(t, focusInput, model.focus, "enter must not switch focus to the list")
	assert.NotEmpty(t, model.SelectedPath(), "enter should record the selected path")
	assert.Contains(t, model.SelectedPath(), memo1.FileName())
}
