...
// Data returns memo file content including metadata, title, body, and headings.
// Order:
//  1. YAML frontmatter with category
//  2. Title
//  3. Top-level body content
//  4. Heading blocks
func (f *MemoFile) Data() []byte {
	meta := f.metadataString()
	rest := f.file.Data()
	// Concatenate metadata and common body
	out := make([]byte, 0, len(meta)+len(rest))
