...
func TestTidyMemos_Success(t *testing.T) {
	// Setup
	tmpDir := t.TempDir()
	repo := NewMemo(tmpDir)
	...
	memo, _ := domain.NewMemoFile(date, "Test Memo", []string{})
...
func TestTidyMemos_EmptyDirectory(t *testing.T) {
	// Setup
	tmpDir := t.TempDir()
	repo := NewMemo(tmpDir)
...
func TestDelete_CleansUpEmptyDirectories(t *testing.T) {
	...
	// Verify the empty directories are cleaned up
	if _, err := os.Stat(categoryDir); !os.IsNotExist(err) {
		t.Error("Empty category directory should be removed after deletion")
	}
}
