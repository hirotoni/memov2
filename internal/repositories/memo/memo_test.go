...
func TestMemoEntries_SkipsUnparseableFiles(t *testing.T) {
	// Setup
	tmpDir := t.TempDir()
	logger := slog.New(slog.NewTextHandler(os.Stdout, nil))
	repo := NewMemo(tmpDir, logger)

	// One well-formed memo
	date := time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC)
	good, _ := domain.NewMemoFile(date, "Good Memo", []string{})
	if err := repo.Save(good, false); err != nil {
		t.Fatalf("failed to save good memo: %v", err)
	}

	// A file whose name matches the memo pattern but whose body has no
	// level-1 heading, so it cannot be parsed into a MemoFile.
	badDate := time.Date(2024, 1, 2, 0, 0, 0, 0, time.UTC)
	badName := badDate.Format(domain.FileNameDateLayoutMemo) + "_memo_broken.md"
	badPath := filepath.Join(tmpDir, badName)
	if err := os.WriteFile(badPath, []byte("no top-level heading here\n"), 0o644); err != nil {
	...
	// Assert: the bad file is skipped, not fatal
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(entries) != 1 {
		t.Fatalf("expected 1 entry (bad file skipped), got %d", len(entries))
	}
	if entries[0].Title() != "Good-Memo" {
		t.Errorf("expected the good memo, got %q", entries[0].Title())
	}
}
