...
func (uc memo) BuildWeeklyReportMemos() error {
	...
	err := uc.r.Memo.TidyMemos()
	if err != nil {
		// continue even if error
		fmt.Print("Error tidying memos: ", err, "\n")
	}

	memos, err := uc.r.Memo.MemoEntries()
	if err != nil {
	...
	for _, memo := range memos {
		...
		if _, week := memo.Date().ISOWeek(); week != prevWeekNum {
			weekHeader := usecase.WeekSplitter(memo.Date())
			e := &domain.HeadingBlock{HeadingText: weekHeader, Level: 2}
			w.SetHeadingBlocks(append(w.HeadingBlocks(), e))
			prevWeekNum = week
		}

		// determine if new date or same as previous
		var e *domain.HeadingBlock
		...
		if sameWithPrevDate {
			...
			order = 1
			e = &domain.HeadingBlock{HeadingText: date, Level: 3} // new entity for new date
		}
	...
	err = uc.r.MemoWeekly.Save(w, true)
	if err != nil {
		return fmt.Errorf("error saving weekly report: %v", err)
	}

	fpath := filepath.Join(uc.c.MemosDir(), w.FileName())
